...
def insertRows(row_len, total_event, prev_row_ptr, array_pos):
    ...
    for loopid in range(0, row_len - 1):
        ...
        db_sd_ev_id = event_org + str(event_id)
        ...
        print('***********************************************************************************************')
        query = "INSERT INTO sd_mma_events (event_name, event_month, event_day, event_year, event_id, event_fight_card_url, event_org, event_location, sd_event_id) VALUES (\"%s\",\'%s\',%s,%s,%i,\"%s\",\"%s\",\"%s\", \"%s\")" % (db_e_en, db_e_ev, db_e_ed, db_e_ey, event_id, db_e_fc, event_org, db_e_lc, db_sd_ev_id)
        print(query)
        print('***********************************************************************************************')
...
event_url = 'http://www.sherdog.com/organizations/Ultimate-Fighting-Championship-2/recent-events/6'
# reset the event id
event_id = 0

ufc_row_len = loadData(event_url, event_org, 1)
...
# set the event url to sherdog ufc section
# event_url = 'http://www.sherdog.com/organizations/Ultimate-Fighting-Championship-2'
event_url = 'http://www.sherdog.com/organizations/Ultimate-Fighting-Championship-2/upcoming-events/1'
# reset the event id
# event_id = 0
ufc_row_len = loadData(event_url, event_org, 0)
ufc_total_events = ufc_total_events + ufc_row_len
print(" ---- Inserts ----")
# insertRows(ufc_row_len, prev_row_ptr, array_pos)
insertRows(ufc_row_len, ufc_total_events, prev_row_ptr, array_pos)
...
bellator_row_len = loadData(event_url, event_org, 1)
...
# reset the event id
# # event_id = 0
bellator_row_len = loadData(event_url, event_org, 0)
bellator_total_events = bellator_row_len + bellator_total_events
# insert the Bellator Data
insertRows(bellator_row_len, bellator_total_events, prev_row_ptr, array_pos)
# set the prev_row_ptrgth pointer
